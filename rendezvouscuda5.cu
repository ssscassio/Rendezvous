...
__device__ int numberFind = 0;

__device__ float **matrizFind;
...
double __device__ brute_E(int N, double x0, double y0, double z0, double xl0, double yl0, double zl0, double Y, int X, double w, int a, double vex, double vey, double vez){
...
__global__ void case1(double x0, double y0, double z0, double xl0, double yl0, double zl0, double w){
  ...
  double vex = blockIdx.y; //ve||vex(Velocidade de exaustão) recebe o valor z atual do thread;
  ...
  if( function >= -0.01 && function <=0.01){
      ve = powf(3,1/2)*vex;
      Found a = { .Y = Y, .X = X, .ve = ve, .diff = function};
      push_back(a);  }
}
...
typedef struct {
    float Y, X, ve, diff} Found;

__device__ Found dev_data[N];
__device__ int dev_count = 0;

__device__ int push_back(Found & found) {
...
int main(){
  ...
  matrizFind = (float**) malloc(sizeof(float*));
  ...
  printf("Valores fixos: x0 = %f; y0 = %f; z0 = %f; xl0 = %f; yl0 = %f; zl0 = %f; w = %f\n\n\n",x0, y0, z0, xl0, yl0 ,zl0, w);
//Colocar o switch aqui futuramente---
  ...
  case 1:
    ...
    std::vector<Found> results(dsize);
    cudaMemcpyFromSymbol(&(results[0]), dev_data, dsize*sizeof(Found));
    for(int i = 0; i <dzise;i++){
      printf("Gama: 10^%d     Chi: %d       ve: %f        diff: %f\n", results[i].Y,results[i].X,results[i].ve, results[i].diff);
    }
    break;
  ...
  case 2:
    ...
    cudaMemcpyFromSymbol(&(results[0]), dev_data, dsize*sizeof(Found));
    for(int i = 0; i <dzise;i++){
      printf("ve: %f     Chi: %d       Gama: 10^%d        diff: %f\n", results[i].ve,results[i].X, results[i].Y, results[i].diff);
    }
    break;
  ...
  case 3:
    ...
    cudaMemcpyFromSymbol(&(results[0]), dev_data, dsize*sizeof(Found));
    for(int i = 0; i <dzise;i++){
      printf("Gama: 10^%d     ve: %f       Chi: %d        diff: %f\n", results[i].Y,results[i].ve,results[i].X, results[i].diff);
    }
    break;
  }
